        ...
        <div class="content-area">
            <div class="card-custom">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h5 class="mb-0"><i class="fas fa-calendar-check me-2 text-primary"></i>Laporan Kegiatan</h5>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php if (empty($kegiatan)): ?>
                                <tr><td colspan="6" class="text-center text-muted py-4"><i class="fas fa-inbox fa-3x mb-3 d-block"></i>Tidak ada kegiatan bulan ini</td></tr>
                            <?php else: ?>
                                <?php foreach ($kegiatan as $keg): ?>
                                    <tr>
                                        <td><?= date('d/m/Y', strtotime($keg['tanggal'])) ?></td>
                                        <td><strong><?= esc($keg['nama_kegiatan']) ?></strong></td>
                                        <td><span class="badge bg-secondary"><?= esc($keg['kategori']) ?></span></td>
                                        <td><?= esc($keg['lokasi']) ?></td>
                                        <td class="text-end">Rp <?= number_format($keg['anggaran'], 0, ',', '.') ?></td>
                                        <td><span class="badge bg-<?= $keg['status'] == 'direncanakan' ? 'warning' : ($keg['status'] == 'selesai' ? 'success' : 'primary') ?>"><?= ucfirst(str_replace('_', ' ', $keg['status'])) ?></span></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
