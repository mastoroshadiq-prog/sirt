        ...
        <div class="content-area">
            <div class="card-custom">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h5 class="mb-0"><i class="fas fa-shield-alt me-2 text-primary"></i>Laporan Ronda</h5>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-md-4"><div class="card text-center"><div class="card-body"><h6 class="text-muted">Total Jadwal</h6><h3><?= count($jadwal) ?></h3></div></div></div>
                    <div class="col-md-4"><div class="card text-center"><div class="card-body"><h6 class="text-muted">Selesai</h6><h3 class="text-success"><?= count(array_filter($jadwal, fn($j) => $j['status'] == 'selesai')) ?></h3></div></div></div>
                    <div class="col-md-4"><div class="card text-center"><div class="card-body"><h6 class="text-muted">Scheduled</h6><h3 class="text-primary"><?= count(array_filter($jadwal, fn($j) => $j['status'] == 'scheduled')) ?></h3></div></div></div>
                </div>
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php if (empty($jadwal)): ?>
                                <tr><td colspan="4" class="text-center text-muted py-4"><i class="fas fa-inbox fa-3x mb-3 d-block"></i>Tidak ada data jadwal ronda</td></tr>
                            <?php else: ?>
                                <?php foreach ($jadwal as $j): ?>
                                    <tr>
                                        <td><?= date('d/m/Y', strtotime($j['tanggal'])) ?></td>
                                        <td><?= $j['shift'] ?></td>
                                        <td><?= esc($j['lokasi_pos']) ?></td>
                                        <td><span class="badge bg-<?= $j['status'] == 'scheduled' ? 'primary' : ($j['status'] == 'selesai' ? 'success' : 'danger') ?>"><?= ucfirst($j['status']) ?></span></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
