            ...
            <div class="row">
                <div class="col-md-10 offset-md-1">
                    <div class="card-custom">
                        ...
                        <form action="<?= base_url('perencanaan/rkt/save') ?>" method="POST">
                            <?= csrf_field() ?>
                            <?php if (isset($rkt)): ?><input type="hidden" name="id" value="<?= $rkt['id'] ?>"><?php endif; ?>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Tahun <span class="text-danger">*</span></label>
                                <input type="number" name="tahun" class="form-control" value="<?= old('tahun', $rkt['tahun'] ?? date('Y')) ?>" required min="2020" max="2050">
                            </div>
                            <div class="row mb-3">
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Periode Mulai <span class="text-danger">*</span></label>
                                    <input type="date" name="periode_mulai" class="form-control" value="<?= old('periode_mulai', $rkt['periode_mulai'] ?? '') ?>" required>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">Periode Selesai <span class="text-danger">*</span></label>
                                    <input type="date" name="periode_selesai" class="form-control" value="<?= old('periode_selesai', $rkt['periode_selesai'] ?? '') ?>" required>
                                </div>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Visi <span class="text-danger">*</span></label>
                                <textarea name="visi" class="form-control" rows="3" required><?= old('visi', $rkt['visi'] ?? '') ?></textarea>
                            </div>
                            <div class="mb-3">
                                <label class="form-label fw-bold">Misi <span class="text-danger">*</span></label>
                                <textarea name="misi" class="form-control" rows="4" required><?= old('misi', $rkt['misi'] ?? '') ?></textarea>
                            </div>
                            <div class="mb-4">
                                <label class="form-label fw-bold">Status</label>
                                <select name="status" class="form-select">
                                    <option value="draft" <?= old('status', $rkt['status'] ?? 'draft') == 'draft' ? 'selected' : '' ?>>Draft</option>
                                    <option value="approved" <?= old('status', $rkt['status'] ?? '') == 'approved' ? 'selected' : '' ?>>Approved</option>
                                    <option value="active" <?= old('status', $rkt['status'] ?? '') == 'active' ? 'selected' : '' ?>>Active</option>
                                    <option value="archived" <?= old('status', $rkt['status'] ?? '') == 'archived' ? 'selected' : '' ?>>Archived</option>
                                </select>
                            </div>
                            <div class="d-grid"><button type="submit" class="btn btn-primary btn-lg"><i class="fas fa-save me-2"></i>Simpan RKT</button></div>
                        </form>
                    </div>
                </div>
            </div>
