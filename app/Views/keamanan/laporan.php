        ...
        <div class="content-area">
            <div class="card-custom">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h5 class="mb-0"><i class="fas fa-exclamation-triangle me-2 text-primary"></i>Laporan Kejadian</h5>
                    <div>
                        <a href="<?= base_url('keamanan/laporan/add') ?>" class="btn btn-primary me-2"><i class="fas fa-plus me-2"></i>Tambah</a>
                ...
                <form method="GET" class="row g-2 mb-3">
                    <div class="col-md-2">
                        <select name="status" class="form-select" onchange="this.form.submit()">
                            ...
                            <option value="dilaporkan" <?= $status_filter == 'dilaporkan' ? 'selected' : '' ?>>Dilaporkan</option>
                            <option value="ditangani" <?= $status_filter == 'ditangani' ? 'selected' : '' ?>>Ditangani</option>
                    ...
                    <table class="table table-hover">
                        ...
                        <tbody>
                            <?php if (empty($list_laporan)): ?>
                                <tr><td colspan="6" class="text-center text-muted py-4"><i class="fas fa-inbox fa-3x mb-3 d-block"></i>Belum ada laporan kejadian</td></tr>
                            <?php else: ?>
                                <?php foreach ($list_laporan as $laporan): ?>
                                    <tr>
                                        <td><?= date('d M Y H:i', strtotime($laporan['tanggal_waktu'])) ?></td>
                                        <td><span class="badge bg-info"><?= esc($laporan['jenis_kejadian']) ?></span></td>
                                        <td><?= esc($laporan['lokasi']) ?></td>
                                        <td><?= esc(substr($laporan['deskripsi'], 0, 50)) ?>...</td>
                                        <td><span class="badge bg-<?= $laporan['status'] == 'dilaporkan' ? 'warning' : ($laporan['status'] == 'ditangani' ? 'primary' : 'success') ?>"><?= ucfirst($laporan['status']) ?></span></td>
                                        <td><a href="<?= base_url('keamanan/laporan/detail/' . $laporan['id']) ?>" class="btn btn-sm btn-info"><i class="fas fa-eye"></i></a></td>
                                    </tr>
                                <?php endforeach; ?>
                            <?php endif; ?>
                        </tbody>
